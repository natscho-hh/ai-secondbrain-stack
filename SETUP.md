...
3. **Stamp the install date.** Replace every `YYYY-MM-DD` placeholder in the copied vault with today's date: `todos.md` ("Last updated"), `.maintenance-log.md` (`last-check`), the `date:` frontmatter of the `00 Context/` files, and the `date:` frontmatter of `01 Inbox/Brain Dump.md` and `02 Projects/Example Project.md` (including its example log entry). A vault that claims it was last updated months before it existed makes the maintenance check misfire on day one.
